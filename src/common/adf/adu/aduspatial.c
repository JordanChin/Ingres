    ...
    switch(GEOSGeomTypeId_r(handle, geometry))
    {
        ...
        case GEOS_POINT:
            for( i = 0; i < sub_geom_count; i++ )
            {
                if(multi)
                {
                    interior = GEOSGetGeometryN_r( handle, geometry, i );
                ...
                GEOSCoordSeq_getX_r( handle, coords, 0, &x );
                GEOSCoordSeq_getY_r( handle, coords, 0, &y );
        ...
        case GEOS_LINEARRING:
            ...
            for( j = 0; j < sub_geom_count; j++ )
            {
                if(multi)
                {
		      interior = GEOSGetGeometryN_r( handle, geometry, j );
                      coords = GEOSGeom_getCoordSeq_r( handle, interior );
                }
                else
		      coords = GEOSGeom_getCoordSeq_r( handle, geometry );
        ...
        case GEOS_POLYGON:
            ...
            for( j = 0; j < sub_geom_count; j++ )
            {
                if(multi)
                {
		    exterior = GEOSGetGeometryN_r( handle, geometry, j );
                    exterior = GEOSGetExteriorRing_r( handle, exterior );
                }
                else
                {
